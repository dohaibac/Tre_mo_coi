...
$(document).ready(function(){
	jQuery.post(
		   ajaxurl, 
		   {
			  action:'load_disqus_recent_comment_widget'
		   }, 
		   function(response){
				$('#disqus_recent_comment_box').html(response);
		   }
		);
});
</script>
EOF;
	...
	public function get_widget_content($args = null, $instance = null) {
		...
		try {
			...
			if(!$comment_length) $comment_length = 150;
	...
	protected function no_comments( $comment = false ) {
	    echo '<div id="disqus_rcw_comment_wrap"><span id="disqus_rcw_no_comments">Không có bình luận nào</span>';
	...
	protected function echo_comments($comment, $api_key, $style_params) {
		...
		if($comment != 'Invalid API key') {
		    ...
		    foreach($comment as $comment_obj) {	      
		      	...
		      	$post_time = time_ago($comment_obj["createdAt"]);
				//$post_time_original = d
				...
				$separator = '';		  
				if($comment_counter != $style_params["comment_limit"])
					$separator = 'border-bottom:1px solid #ECE8DC;';
				
		      	//create comment html
		      	$comment_html = '<div class="disqus_rcw_single_comment_wrapper">
		                <div style="'.$separator.'padding-bottom:15px;padding-top:10px;">
		                  <div class="disqus_rcw_comment_actual_wrapper">
						  <div style="float:left;margin-right:5px;margin-bottom:5px;"><img style="width:32px;height:32px;" class="disqus_rcw_avatar" src="'.$author_avatar.'" alt="'.$author_name.'"/></div>
		                  <span style="font:normal large serif;">&#x201C;</span><a style="font-size:12px;font-style:normal;color:#222222;" href="'.$thread_link.$comment_id.'">'.$message.'</a><span style="font:normal large serif;">&#x201D;</span> <span style="font-size:12px;">đăng bởi</span> <a href="'.$author_profile.'" style="font-weight:bold;color:#A9525B;font-style:normal;">'.$author_name.'</a> <span style="font-size:12px;">'.$post_time.'</span>
		                  </div>
						  <div style="margin-top:5px;"><a class="disqus_rcw_thread_title" style="font-weight:normal;" href="'.$thread_link.'">'.$thread_title.'</a></div>
		                </div>
		              </div>';
...
add_action('wp_ajax_load_disqus_recent_comment_widget', 'load_disqus_recent_comment_widget');

function load_disqus_recent_comment_widget(){
	$widget =  new disqus_recent_comments_widget();
	$widget->get_widget_content();
	die();
}
