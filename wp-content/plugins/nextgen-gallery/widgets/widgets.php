	...
	function widget( $args, $instance ) {
		...
		if (is_array($imageList)){
			foreach($imageList as $image) {
				...
				$out = '<li style="height:154px;margin-right:10px;margin-left:10px;overflow:hidden;"><a href="' . $image->imageURL . '" title="' . $description . '" ' . $thumbcode .'>';
				...
				if ( $instance['show'] == 'original' )
					$out .= '<img class="thumb" src="' . trailingslashit( home_url() ) . 'index.php?callback=image&amp;pid='.$image->pid.'&amp;width='.$instance['width'].'&amp;height='.$instance['height']. '" title="'.$alttext.'" alt="'.$alttext.'" />';
				else	
					$out .= '<img class="thumb" src="'.$image->thumbURL.'" width="'.$instance['width'].'" height="'.$instance['height'].'" title="'.$alttext.'" alt="'.$alttext.'" />';			
				
				echo $out . '</a><h3><a href="#">'.$alttext.'</a></h3></li>'."\n";
